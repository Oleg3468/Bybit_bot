...
async def notify(app, text: str):
    if ALLOWED_USER_ID:
        try: await app.bot.send_message(chat_id=ALLOWED_USER_ID, text=text, parse_mode="Markdown")
...
async def auto_trader_loop(app):
    while True:
        try:
            trader = AutoTrader(
                engine_fn=engine, deposit=DEPOSIT, risk_pct=RISK_PCT, leverage=LEVERAGE,
                notify_fn=lambda text: notify(app, text),
            )
            logger.info("🤖 AutoTrader инициализирован.")
            break
        except Exception as e:
            logger.error(f"AutoTrader init error: {e}. Retrying in 30s...")
        ...
        try:
            ...
            if not signals:
                ...
                for signal in signals:
                    if trader.notify_fn:
                        await trader.notify_fn(f"📡 *Найден сигнал!*\n{signal.format()}\n\n⏳ Исполняю...")
                    
                    result = await trader.execute_signal(signal)
                    
                    if trader.notify_fn:
                        if result.get("ok"):
                            await trader.notify_fn(f"✅ *Сделка открыта автоматически!*\n{signal.symbol} {signal.side}\nRR: 1:{signal.rr}")
                        else:
                            await trader.notify_fn(f"❌ Ошибка исполнения: {result.get('msg')}")
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd in ("лонг","long","шорт","short"):
        ...
        price = await loop.run_in_executor(None, engine().get_price, sym)
...
async def handle_auto(update: Update, parts: list):
    if len(parts) < 2:
        ...
        await update.message.reply_text(f"🤖 *Автоторговля*\nСтатус: {status}\n\n`авто вкл` — включить\n`авто выкл` — выключить\n`авто скан` — один проход сейчас", parse_mode="Markdown")
        return
    ...
    if cmd2 in ("вкл","on","включить","start"):
        ...
        try:
            trader = AutoTrader(engine_fn=engine, deposit=DEPOSIT, risk_pct=RISK_PCT, leverage=LEVERAGE)
            signals = await trader.scan_once()
            if not signals: await update.message.reply_text("📭 Сигналов не найдено. Рынок не даёт входа.")
            else:
                for sig in signals: await update.message.reply_text(sig.format(), parse_mode="Markdown")
        except Exception as e: await update.message.reply_text(f"❌ Ошибка скана: {e}")

async def handle_trade(update: Update, parts: list, side: str):
    if len(parts) < 2: await update.message.reply_text("❌ Формат: `лонг ada` или `лонг ada 0.385 0.370 0.430`", parse_mode="Markdown"); return
    sym, eng, mode = parse_symbol(parts[1]), engine(), current_mode["mode"]
    loop = asyncio.get_event_loop()
    
    entry = await loop.run_in_executor(None, eng.get_price, sym)
    ...
    result = await loop.run_in_executor(None, partial(eng.place_order, symbol=sym, side=side, qty=decision.qty, sl=decision.sl, tp=decision.tp, leverage=decision.leverage))
...
async def handle_close(update: Update, parts: list):
    ...
    result = await loop.run_in_executor(None, engine().close_position, sym)
...
async def handle_positions(update: Update):
    loop = asyncio.get_event_loop()
    data = await loop.run_in_executor(None, engine().get_positions)
    if not data["ok"]: await update.message.reply_text(f"❌ {data['msg']}"); return
    positions = data["positions"]
    if not positions:
        sess = get_current_session(); await update.message.reply_text(f"📋 Нет позиций {mode_badge()}\n{sess['emoji']} {sess['name']}"); return
